...
final class OpenAIProvider: LLMProvider, @unchecked Sendable {
        ...
        let payload: [String: Any] = [
            ...
            "response_format": [
                ...
                "json_schema": [
                    "name": "intake_turn",
                    "strict": true,
                    "schema": IntakePrompt.strictTurnSchema,
                ],
            ],
        ]
    ...
    private func parseChatResponse(data: Data) throws -> ChatTurnResult {
        ...
        guard let turnKind = obj["turn_kind"] as? String else {
            // Fall back to heuristic decode (model may ignore the envelope).
        ...
        case "card":
            ...
            return .card(card)
        default:
            throw LLMError.decodingFailed("unknown turn_kind: \(turnKind)")
        }
    }
}
